...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < N; i++) {
        fprintf(data, "%d ", up[i]);
	/////КОПИРУЮ В НЕГО ЗНАЧЕНИЯ НЕОТСОРТИРОВАННОГО МАССИВА
	test[i] = up[i]; 
    }
    ///CОРТИРУЮ ТЕСТОВЫЙ МАССИВ С QSORT 
    ...
    fclose(data);
    ///СРАВНИВАЮ РЕЗУЛЬТИРУЮЩИЙ МАССИВ С ТЕСТОВЫМ
    /// ЕСЛИ ЧТО-ТО НЕ СОВПАДАЕТ ТО В КОНСОЛЬ ВЫВОДИТСЯ ИНДЕКС 
    for (int i = 0; i < N; ++i) {
	if (test[i] != res[i]) {
		printf("error: %d \n", i);
	}
